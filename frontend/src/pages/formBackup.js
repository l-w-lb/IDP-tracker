...
function Form() {
  ...
  const handleTimeChange = (partIndex, topicIndex, childIndex, questionIndex, groupInstance, subInstance, newAnswer, isChild) => {
    setFormData(prevFormData => {
      const updated = [...prevFormData];
      const questions = isChild
        ? [...updated[partIndex].topics[topicIndex].children[childIndex].questions]
          ...
          <input
            ...
            onChange={(e) => {
              ...
              let [hr, mm, ss] = val.split(":");
                ...
                // const formatted = `${hrStr}:${mmStr}:${ssStr}`;
                const formatted = val;
  ...
  const renderTopicQuestions = ({
    ...
    } else if (topicElement.type === "dinamicQuestion") {
      return (
        <div>
          {topicElement?.questions?.map((question, questionIndex) => {
            // console.log(question)
